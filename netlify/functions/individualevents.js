...
export async function handler(event) {
  ...
  try {
    ...
    rsvps.forEach((row) => {
      ...
      events.push({
        name: row["Event Name"] || "",
        slug: row["Event Slug Webflow"] || "",
        image: row["Image URL"] || "",
        date: row["created Time"] || "", // Airtable field ka naam wahi use kiya hai jo data me tha
        points: parseInt(row["Points Earned"] || 0, 10),
      });
    });
    ...
    const result = {
      ...
      eventSlugs,
      events, // ✅ new detailed events array
    };
